/**
 * lib/lead-magnet-bio/email.ts
 *
 * Email J0 du tool "générateur de bio Instagram coach sportif".
 * Livre les 5 bios générées + tease Kooach Pro.
 */

import { env } from '@/lib/env';
import type { BioItem } from '@/lib/lead-magnet-bio/schema';
import { BIO_CATEGORY_LABELS } from '@/lib/lead-magnet-bio/schema';

interface SendBiosEmailInput {
  to: string;
  niche: string;
  bios: BioItem[];
}
...
function renderBio(bio: BioItem, idx: number): string {
  const label = BIO_CATEGORY_LABELS[bio.category] ?? bio.category;
  // Préserve les sauts de ligne dans la bio (Insta supporte le multi-line)
  const text = escapeHtml(bio.text).replace(/\n/g, '<br/>');
  return `
    <tr>
      <td style="padding:14px 0;border-bottom:1px solid #E8E8E2">
        <p style="margin:0 0 6px;font-family:Georgia,serif;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#2D6A4F">
          ✦ ${String(idx + 1).padStart(2, '0')} · ${escapeHtml(label)}
        </p>
        <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:15px;line-height:1.55;color:#111827;white-space:pre-line">
          ${text}
        </p>
      </td>
    </tr>`;
...
export async function sendLeadMagnetBiosEmail({ to, niche, bios }: SendBiosEmailInput): Promise<void> {
  const subject = `Tes 5 bios Instagram ✦ ${niche}`;
      ...
      <tr>
        <td align="center" style="padding:40px 20px">
          <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width:600px;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(45,106,79,0.08)">
            ...
            <tr>
              <td style="padding:18px 36px 8px">
                <h1 style="margin:0;font-family:Georgia,serif;font-size:28px;font-weight:700;line-height:1.15;color:#111827;letter-spacing:-0.8px">
                  5 bios pour ton compte Instagram.
                </h1>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:8px 36px 24px">
                <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:15px;line-height:1.65;color:#6B7280">
                  Pour ta niche : <strong style="color:#111827">${escapeHtml(niche)}</strong>.<br/>
                  Choisis-en une, copie-la, colle-la sur Instagram. Tu peux aussi en mixer 2 (pioche le hook de l'une, le CTA de l'autre).
                </p>
              </td>
            </tr>
                  ...
                  <tr>
                    <td style="padding:14px 18px">
                      ...
                      <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;line-height:1.6;color:#1a5c38">
                        Avant de coller la bio, vérifie qu'elle tient en 150 caractères max sur ton compte. Si tu utilises beaucoup d'emojis, retire ce qui n'est pas essentiel — la lisibilité prime.
                      </p>
                    </td>
                  </tr>
                </table>
                  ...
                  <tr>
                    <td style="padding:24px 22px">
                      ...
                      <h2 style="margin:0 0 10px;font-family:Georgia,serif;font-size:20px;font-weight:700;line-height:1.25;color:#111827;letter-spacing:-0.4px">
                        7 contenus Insta calibrés sur ton style — en 60 secondes.
                      </h2>
                      <p style="margin:0 0 18px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;line-height:1.65;color:#6B7280">
                        Avec <strong style="color:#111827">Kooach Pro</strong>, tu génères <strong style="color:#111827">3 posts + bio + newsletter + email + idée réel</strong> en une seule génération. Calibrés sur ton ton, ta cible, ton objectif. Pas une IA générique.
                      </p>
                      <a href="https://kooach.fr/signup?utm_source=lead-magnet&utm_medium=email&utm_campaign=bio-j0"
            ...
            <tr>
              <td style="padding:18px 36px 28px;border-top:1px solid #E8E8E2;background:#FAFAF7">
                <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:12px;line-height:1.6;color:#9CA3AF;text-align:center">
                  Kooach · Le ghost-writer IA des coachs sportifs FR<br/>
                  <a href="https://kooach.fr" style="color:#2D6A4F;text-decoration:none">kooach.fr</a>
                </p>
              </td>
            </tr>
          </table>
